# Cartograph — Post-Compaction Recollection (2026-05-04, post-DEMO-MEGA)
...
1. **Phase 10.1.3** — rename params to `agent_id` across 5 tools:
   - `send_chat`: `from_agent_id` → `agent_id`
   - `send_broadcast`: `from_agent_id` → `agent_id`
   - `create_task`: `owner_agent_id` → `agent_id` (keep `worker_agent_id` separate)
   - `act_on_proxy_item`: `survivor_id` → `agent_id`
   - 5th tool: confirm via insight `5e58b07d` (likely `raise_blocker`)
   - Plus: `create_edge` docstring fix — currently says "Refuses self-loops" which is wrong (Phase 7.3 + 7.4.4 dropped that constraint). 5 LOC.

2. **DEMO10 targeted (~7 phases, 10-15 min):**
   - Tool surface 114
   - Direct-call regression on each renamed tool (new param works)
   - mcp_call_batch heterogeneous batch with all 5 renamed tools as sub-calls — every sub-call ok=True (the bug 10.1.3 fixes)
   - Spot-check existing scenarios (chat / broadcast / task / proxy / blocker)
   - Caveman + lock-step still bite (regression)
   - Bonus: `create_edge` self-loop succeeds + docstring grep shows no "Refuses self-loops" line
   - Scorecard

3. **Order to execute:**
   1. Read insight `5e58b07d` to confirm 5th tool
   2. Implement rename + docstring fix incrementally (commit per tool cluster)
   3. Run existing test suite at each step
   4. Sweep prompt + doc references for `from_agent_id` / `owner_agent_id` / `survivor_id`
   5. Doc sync commit
   6. Write DEMO10 prompt to `docs/oorch-test-prompt-demo10`
   7. Wipe DB (preserving `src/workspaces.bak.20260426-2122/` real-data backup), restart 4 daemons
   8. Hand DEMO10 to fresh orch
   9. User invokes `/loop` to monitor

### Past DEMO state (just to remember):

DEMO-MEGA scorecard at `docs/oorch-test-prompt-demoMEGA`. Final scorecard message in DB at:
```sql
SELECT text FROM communications WHERE from_agent='orch-5d2a0fa8' AND text LIKE '%FINAL SCORECARD%' ORDER BY created_at DESC LIMIT 1;
```

---

## 1. Branch + commit state (HEAD as of 2026-05-04, post-DEMO-MEGA)
...
- **HEAD:** `e1574bb` (DEMO-MEGA prompt commit). Pushed to `origin/feat/trigger-manager-cartograh-mcp`.

Recent commits, newest first:
```
e1574bb DEMO-MEGA: 31-phase end-to-end verification covering Phase 0-10
fc7a742 docs: Phase 10 sync — 10.1.1 + 10.1.2 + caveman rewrite landed
...
20480b3 Phase 10.5: doc sync pass — 108 → 114 tools, search tools registered
...
8100a91 docs: Phase 10 plan
273858b docs: catch-up sync — wake debounce window 5 min → 1 min
bc72bd0 trigger_loop: WAKE_DEBOUNCE_SECONDS 300 → 60
66e5709 DEMO9: tighten Phase 4a partial-failure verification
fedae33 Phase 9.4: DEMO9 orchestration prompt — Phase 9 verification
211d840 Phase 9.3: doc sync pass — 107→108 tools, caveman supersedes concise rule
...
b587055 docs: Phase 9 plan — Round 5 token-opt
```
...
End of recollection. Feed this + the 7 docs + memory file (auto-loaded) and I'll be caught up. Most important sections to scan first: §0 (current state + next move), §6 (DEMO-MEGA results), §11-12 (Phase 10.1.3 + DEMO10 plan).

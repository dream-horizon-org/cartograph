...
- **10.1 Symmetric-nomination race guard (commit `2a62836`):** pre-INSERT SELECT in `nominate_consolidation` refuses a duplicate open merge consolidation between the same component pair (either direction, status NOT IN D/F). Skips for splits. Closes the SME-A↔SME-B simultaneous-nomination collision.
...
- **Sleep — LAST RESORT, not a default.** Default behaviour when drained: just yield. The trigger scanner only re-wakes on real work; yielding doesn't burn cost. Sleep ONLY when blocked on admin/external + already prompted twice; 300–600s MAX, never 24h, never >3600s. Applied across SME / iterator / orchestrator prompts.

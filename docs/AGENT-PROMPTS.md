...
**Phase 10.13 — Post-real-data insight triage bundle (SHIPPED 2026-05-08, 9 commits):** synthesised from 3× real-data onboarding runs (2026-05-06) + 56 OPEN insights. Ten items across 3 tiers; tool count 114 → 117; 1 schema migration; 13 insights triaged (10 promoted, 3 wontfix).
  ...
  - **10.13.12 Iterator ADMIN-SCOPE rule (commit `8c7fd30`, 2026-05-11):** new block at top of "YOUR JOB (iteration phase)" in `iterator.py`. If admin onboarding gives a narrow target list, treat it as exhaustive scope: `upsert_resource(s_bulk)` writes ONLY for (named targets ∪ dependency-linked datastores/caches/queues/external services). Full plane enumeration dumps to a workspace file (`./<plane>_seen.json`) for cross-reference — NOT mega-dumped to `resources`. Report `seen N / upserted M / filtered (N-M)` in task response. Handle plane-native name variants (e.g. `fantasy-tour` → `fantasy-tour-v1` in APM) by upserting plane-native + noting alias in metadata. Trims ~50 LOC of admin kickoff boilerplate per run.
  - **10.13.13 SME lanes 8 → 12 (commit `89ec1c4`, 2026-05-11):** `INVOKE_LANES_SME` default 8 → 12. Total subprocess concurrency 12 → 16 (orch 1 + iter 2 + res 1 + sme 12). Ahead of 4th real-data run where 9+ SMEs across two planes will spawn concurrently.
  - **10.14.1 drop TEMP lock-step doctrine (commit `beeba19`, 2026-05-12):** removed the 5-phase lock-step block from `base.py::MISSION_AND_VOCABULARY` + the phase-coordinator section from `orchestrator.py`. SMEs were reading "stay DANGLING during MATERIALISATION" as "no cross-plane discovery at all" — caused F1 (multiple unmerged cross-plane sibling components in run #4). Replaced with a brief "cross-plane sibling search is encouraged" rule; edge BINDING still deferred per asymmetric edge protocol. Companion O1 (orch waiting for greenlight on phase advance) became moot.
  - **10.14.2 spawn_child_agent mints fresh agent_id (commit `f905384`, 2026-05-12):** server generates `sme-<8hex>` via `_mint_fresh_agent_id()` with collision-retry against `agent_runs`. Caller-supplied `child_agent_id` was the root cause of the #8 wedge class (3 SMEs in run #4 ended up owning 2 active components). Legacy param raises ValueError.
  - **10.14.3 `consolidations.cascade_completed_at` guard (commit `008c208`, 2026-05-12):** new nullable TIMESTAMPTZ column. `absorb_agent` and `spawn_child_agent` stamp on success. `execute_mutation` refuses M→MD if NULL — closes the F3 silent-corruption class observed on cons `a21f113a` in run #4 (sme-5c9dfcf6 fired execute_mutation before absorb_agent).
  - **10.14.4 attribution cascade auto-dedup + delete cascade_* flags (commit `a96127a`, 2026-05-12):** `transfer_attributions` per-row dedup loop replaces bulk UPDATE — on collision, keep survivor's row, merge target metadata, MAX confidence, drop target's row. Direct mirror of Phase 10.13.8's pattern for edges. Closes F2 frozen-attrs class (2 cases in run #4: fantasy-tour-admin-telemetry 11 attrs + fantasy-tour-admin-aurora-reader 5 attrs frozen on tombstones because SMEs used `cascade_attributions=False` workaround). The boolean cascade flags were removed entirely — cascade is unconditional; passing any of the flags raises ValueError.
  - **10.17 [ADMIN-HACK-ORDERS-INFERRING] doctrine-conflict fix (commit pending, 2026-05-12 night):** First run-#5 attempt revealed 3 SMEs honestly reported they DIDN'T fire inferred-split nominations despite the Phase 10.16 block being in their prompt. Diagnosis: older Phase 10.8.3 "LEAVE DANGLING — DO NOT FORCE-CREATE THE TARGET" rule at STEP 3 (workflow site) overrode the newer INFERRING block (later in prompt). Agents read STEP 3, satisfied safer-by-default rule, didn't act on override. Plus: vector_search was being run with semantic/free-text descriptions ("MySQL database demo") not identifier-verbatim → false-negatives. Fix: at the STEP 3 leave-dangling site, inserted a `★★★ HARD EXCEPTION — INFERRED NON-CODE STUBS ★★★` sub-block IMMEDIATELY after the default statement; inline trigger pattern examples (`*-mysql-*.dream11.local`, `redis://`, kafka bootstrap, `otlp.last9.io`, etc.); explicit mandatory 4-step sequence ending with "treating insert_unresolved as terminal step is the BUG." In the INFERRING block: clarified pre-flight vector_search MUST use `name_pattern=<IDENTIFIER VERBATIM>`, never semantic descriptions (semantic ranking on description+name false-negates because inferred stubs' canonical_name IS the raw identifier). ILIKE wildcards only for templated env-var placeholders. Companion: DB wiped + daemons restarted so next agent wake loads corrected prompt.
  - **10.16 [ADMIN-HACK-ORDERS-INFERRING] inferred-component spawn (commit `c4e7a02`, 2026-05-12 night):** Two prompt blocks (sme.py + resolver.py) implementing a TEMP HACK for single-plane runs to get usable graphs. When a caller-SME discovers an outbound dangling to a non-code resource (DB/cache/queue/topic/broker) and no other plane will materialise the target, the SME nominates a `type='split'` consolidation with `metadata.admin_hack='inferring'` + `metadata.inferred=true` + `metadata.inferred_kind` + `metadata.inferred_identifier`, message tagged `[ADMIN-HACK-ORDERS-INFERRING]`. Resolver pre-flight: vector_search for existing inferred stub → refuse-with-pointer if found; serialise concurrent noms for same identifier (approve earlier, refuse later with "wait for cons X to land"); ordering check (inferred-splits run AFTER all real splits, BEFORE any merges); metadata validation; then approve to M. Child SME spawned via `spawn_child_agent` hydrates from parent's repo (clone same github resource), parent's attributions (`get_attributions`), and grep for the identifier in adjacent config files. Child component gets `canonical_name = identifier` (verbatim, per Phase 10.15.1 abbreviation guard), confidence 0.5-0.8, attributions for hostname/runtime/schema/port/db_system, NO catalogs, NO outbound edges. Parent binds its dangling edge + resolves its unresolved row to the child once it lands at D. Tagged + grep-able for cleanup when the real inferred-component paradigm ships.
  - **10.15 P1/P2 prompt polish bundle (commit `a49b2ed`, 2026-05-12 evening):** 5 prompt-only rules added per admin verdict on remaining run #4 items. (a) `sme.py` SPLIT/MERGE DISCIPLINE section gains: abbreviation hallucination guard (canonical_name stays verbatim with the observed APM identifier — never expand `ft-cm-poller` to "Fantasy Tour Contest Management Poller"; cross-plane merge cascade is the only path to a richer name); monorepo container dissolution after full carve-out (Option A — distribute root build files into each child's source_slice + decommission container, NOT leave a hollow `application` row with only build scripts); cluster doctrine (Aurora master+reader / Redis cluster / RDS Multi-AZ = ONE component with N endpoint hostnames as separate `attribution` rows, `metadata.role` distinguishes master/reader; Aurora worked example inline); telemetry bare-label low-confidence placeholder (when Last9 emits bare `redis`/`mysql` without hostname, materialise at confidence=0.5 + `metadata.awaiting_hostname_corroboration=true` with `<caller>-<type>-unknown` canonical_name; don't try to merge — cross-plane rule catches it organically). (b) `base.py::MISSION_AND_VOCABULARY` gains a `RAISE_BLOCKER FOR TOOL-LEVEL ERRORS` block — for wedged-state / UNIQUE violations / MCP unreachable / etc., prefer `raise_blocker(task_id, detail)` over `send_chat(to='admin')` so the issue surfaces in the tasks/BO pipeline + auto-re-wakes the agent on resolution. Closes run #4's 4 informal chat escalations as a pattern. Tool count unchanged at 117.
...
- Act — mutation (gated: `status='M' AND mutation_assigned_to == you`): `execute_mutation` (M→MD — **Phase 10.14.3: refuses if `cascade_completed_at IS NULL`**), `absorb_agent(consolidation_id, target, deactivation_reason?, deactivation_notes?)` — **Phase 10.14.4: cascade unconditional + auto-dedup on (plane, rt, id) collision (keep survivor's row, merge target metadata, MAX confidence, drop target's). `cascade_attributions` / `cascade_edges` / `cascade_flows` flags REMOVED — callers passing them raise ValueError. Stamps `cascade_completed_at` on success.** Catalog cascade still runs BEFORE flow cascade. `spawn_child_agent(consolidation_id, child_component_data, child_source_slice, split_briefing, transfer_edge_ids?, transfer_flow_ids?, transfer_attribution_ids?, transfer_catalog_ids?)` — **Phase 10.14.2: server mints fresh `child_agent_id` (returned in response); caller does NOT pass it. Stamps `cascade_completed_at` on success.** Atomic split carve-out + welcome BW task. `transfer_attributions(consolidation_id, ...)`, `transfer_edges(consolidation_id, edge_ids, direction='from'|'to'|'both')`, `transfer_flows(consolidation_id, flow_ids)`.

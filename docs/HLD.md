...
| `absorb_agent(agent_id, cons_id, target_id, reason?, notes?)` | — | — | ✓ *mutation POC* | — | Merge. Populates deactivation cols + unions source_slice + re-points RCA. **Phase 10.14.4: cascade is unconditional** — attributions/edges/flows/catalogs always cascade from target to survivor. `transfer_attributions` auto-dedups on (plane, resource_type, identifier) collision (keep survivor's row, merge target metadata, MAX confidence, drop target's row) — direct mirror of Phase 10.13.8 for edges. The legacy `cascade_attributions` / `cascade_edges` / `cascade_flows` boolean flags were removed; callers passing any raise `ValueError`. Catalog cascade still runs unconditionally BEFORE flow cascade. **Phase 10.14.3:** stamps `consolidations.cascade_completed_at = now()` on success — required by `execute_mutation` (M→MD refuses if NULL). Chain never flattened. |
| `spawn_child_agent(parent_id, cons_id, comp_data, slice, briefing, transfer_edge_ids?, transfer_flow_ids?, transfer_attribution_ids?, transfer_catalog_ids?)` | — | — | ✓ *mutation POC* | — | Split. Atomic carve-out. **Phase 10.14.2: server mints fresh `child_agent_id` server-side** — caller no longer supplies it (previously enabled spawn-collision wedges where one SME ended up owning 2 active comps). Returned dict carries the minted `child_agent_id`. Callers passing the legacy param raise `ValueError`. Phase 4.1: strict top-level source_slice guard, welcome BW task, optional edge/flow transfers. Phase 4.2: optional attribution transfers (mandatory hygiene — without it, hostname-class attributions strand on parent). Phase 7.4.2: optional `transfer_catalog_ids` to move catalog rows parent→child. **Phase 10.14.3:** stamps `consolidations.cascade_completed_at` on success. Consolidation.child_agent_id blocks re-spawn. |

...
| `absorb_agent(agent_id, cons_id, target_id, reason?, notes?, cascade_attributions=True, cascade_edges=True, cascade_flows=True)` | — | — | ✓ *mutation POC* | — | Merge. Populates deactivation cols + unions source_slice + re-points RCA. Phase 4.1 cascades move attributions/edges/flows from target to survivor via the scoped transfer tools (defaults on). Chain never flattened. |
| `spawn_child_agent(parent_id, cons_id, child_id, comp_data, slice, briefing, transfer_edge_ids?, transfer_flow_ids?)` | — | — | ✓ *mutation POC* | — | Split. Atomic carve-out. Phase 4.1: strict top-level source_slice guard, welcome BW task for child, optional edge/flow transfers in same call. Consolidation.child_agent_id blocks re-spawn. |
...
| `get_stale_flows(agent_id)` | — | — | ✓ | — | Phase 4.1 hygiene. Flows with a dead-component edge endpoint. |

...
| `spawn_child_agent(parent_id, cons_id, child_id, comp_data, slice, briefing, transfer_edge_ids?, transfer_flow_ids?, transfer_attribution_ids?)` | — | — | ✓ *mutation POC* | — | Split. Atomic carve-out. Phase 4.1: strict top-level source_slice guard, welcome BW task, optional edge/flow transfers. Phase 4.2: optional attribution transfers (mandatory hygiene — without it, hostname-class attributions strand on parent). Consolidation.child_agent_id blocks re-spawn. |

...
def test_boot_creates_singleton_agents(tmp_project):
    trigger_mgr = boot(
    ...
    # Should have orchestrator and resolver
    agents = []
    ...
    assert "resolver" in agent_types
...
def test_boot_is_idempotent(tmp_project):
    ...
    # Should still have only 2 agents (singletons)
